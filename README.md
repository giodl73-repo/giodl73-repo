...
The fairest method turns out to be the oldest one in the book: **divide in half.** Split each state into two halves of equal population; split each half again; keep going until you have the right number of districts. Nobody gets to choose which half is theirs, so nobody gets an advantage — every district is the product of a series of neutral halvings, not a single optimized design. The state becomes a graph (census tracts as nodes, shared borders as edges weighted by border length) and **METIS** runs recursive bisection under a **±0.5% population-balance** constraint. **No political or racial data enters at any stage.**
...
Around the algorithm sits the same AI-first tooling as the rest: **20 research papers across five tracks** (Synthesis, Algorithm, Validation, VRA, Experimental Extensions), an interactive 50-state dashboard, a 215-test suite, and a LaTeX pipeline that compiles papers, presentations, and guides directly from the run outputs. Different subject matter; same discipline.
